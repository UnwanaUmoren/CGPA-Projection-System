...
$(document).ready(function(){
	...
	OutstandingTable = $(".con div.m_con_right div.c_table_con table#out_table");
	
	coursesArray();
	
	oustandingCoursesArray()
	validate();
	scan_row();
	lockUpTable();
	update();
...
function oustandingCoursesArray(){
	OutstandingTable.find("tr").each(function(indx){
		if (indx != 0){
			cCode = $(this).find("td:eq(2)").text();
			OutstandingCourses[indx-1]	= cCode;
		}
	})	
}
function validate(){
...
/////////////////////////////________Checkbox__________//////////////////////////////
	coursesConTable.on("click", "input[type='checkbox']",function(event){
		if (upCoursesCode.length==0){
			...
			}else if (OutstandingCourses.indexOf(cCode) != -1){
				event.preventDefault();
				alert("You cannot add an oustanding course");
